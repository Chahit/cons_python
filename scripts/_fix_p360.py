"""
Fix partner_360.py: replace broken churn block (lines 168-332, 1-indexed)
with valid Python — no backslash escapes inside f-strings.
"""
    ...
    if churn_signals and _seg in ("At Risk", "Critical"):
        ...
        for _cn, _cv in score_breakdown.items():
            ...
            _sub_html += (
                ...
                "<div style='width:" + str(_sp) + "%;background:" + _rl + ";border-radius:4px;height:6px;'>"
                "</div></div>"
                "<div style='font-size:11px;color:" + _rl + ";width:22px;'>" + str(int(_cv)) + "</div>"
                "</div>"
            )
        ...
        st.markdown(
            ...
            "Why this partner is at risk &mdash; peer context &amp; next steps</div></div>"
            "<div style='text-align:center;background:" + _rl + "18;border:1px solid " + _rl + "44;"
            ...
            "</div></div></div>"
            + _bar_html +
            "<div style='font-size:11px;color:#64748b;margin-bottom:12px;'>" + risk_label + "</div>"
            "<div style='background:#161b2a;border-radius:8px;padding:12px;'>"
            "<div style='font-size:11px;color:#94a3b8;font-weight:600;margin-bottom:8px;"
            "text-transform:uppercase;letter-spacing:0.5px;'>Score Breakdown</div>"
            + _sub_html +
            "</div></div>",
        ...
        if top_signal:
            ...
            st.markdown(
                ...
                + _ts_detail +
                "</div><div style='background:#ffffff0a;border-radius:6px;padding:10px 12px;'>"
        ...
        if _others:
            ...
            for s in _others:
                ...
                _chips_html += (
                    ...
                    "background:" + _sc + "18;color:" + _sc + ";"
                    "border:1px solid " + _sc + "44;"
                    "font-size:12px;font-weight:600;margin:3px;'>" + _sig_name + "</span>"
                )
            st.markdown(
                "<div style='margin-bottom:14px;'>"
                "<div style='font-size:11px;color:#64748b;font-weight:600;"
                "text-transform:uppercase;letter-spacing:0.5px;margin-bottom:6px;'>"
                "Other Contributing Signals</div>"
                + _chips_html + "</div>",
                unsafe_allow_html=True,
            )
        ...
        if peer_ctx:
            ...
            _cl  = peer_ctx.get("cluster_label", "Cluster")
            _pcnt = peer_ctx.get("peer_count", 0)
            _tr  = peer_ctx.get("this_rev_fmt", "Rs0")
            _pr  = peer_ctx.get("peer_avg_rev_fmt", "Rs0")
            _tc2 = peer_ctx.get("this_cats", 0)
            _pc2 = peer_ctx.get("peer_avg_cats", 0)
            _rec = peer_ctx.get("peer_avg_recency", 0)
            st.markdown(
                ...
                "Peer Benchmark &mdash; " + str(_cl) + " (" + str(_pcnt) + " peers)</div>"
                "<div style='text-align:center;'>"
                "<div style='font-size:20px;font-weight:700;color:#e2e8f0;'>" + str(_tr) + "</div>"
                "<div style='font-size:11px;color:#64748b;'>This partner (90d)</div></div>"
                "<div style='font-size:20px;color:#334155;'>vs</div>"
                "<div style='text-align:center;'>"
                "<div style='font-size:20px;font-weight:700;color:#94a3b8;'>" + str(_pr) + "</div>"
                ...
                + str(int(_tc2)) + " vs " + f"{_pc2:.0f}" + " avg</span><br/>"
                "Peer avg last purchase: " + f"{_rec:.0f}" + "d</div>"
                "</div>",
                unsafe_allow_html=True,
            )
        ...
        with _r1:
            st.markdown(
                ...
                + str(rec_label) +
                "</div>"
                "<div style='font-size:11px;color:#64748b;margin-top:6px;'>Estimated Yearly</div>"
...
# Find start: first line containing "Churn Risk Intelligence v2"
start_idx = None
for i, l in enumerate(lines):
    if "Churn Risk Intelligence v2" in l:
        start_idx = i
        break

# Find end: first "st.markdown" with "ui-section" AFTER start
end_idx = None
if start_idx is not None:
    for i in range(start_idx + 1, len(lines)):
        if "ui-section" in lines[i] and "st.markdown" in lines[i]:
            end_idx = i
            break

print(f"start_idx={start_idx}  end_idx={end_idx}")
if start_idx is None or end_idx is None:
    raise RuntimeError("Could not locate block boundaries — check file manually.")

new_lines = lines[:start_idx] + [CLEAN_BLOCK + "\n"] + lines[end_idx:]
