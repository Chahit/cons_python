...
def _compute_prior_window_segments(
    ...
    if span_days <= 45:
        resolved_period = "Monthly"
    elif span_days <= 180:
        resolved_period = "Quarterly"
    else:
        resolved_period = "Yearly"

    df_prior = _recompute_segments_for_period(df_prior, rev_mult, period=resolved_period)
...
def render(ai):
    ...
    _seg_key = f"seg_{_pf_hash}_{sel_start}_{sel_end}_{resolved_period}_{hash(_filter_sig) % 999999}"
    if _seg_key not in st.session_state:
        st.session_state[_seg_key] = _recompute_segments_for_period(df, rev_mult, period=resolved_period)
    ...
    # ── Simulator & Threat Matrix Upgrades (Wow Factors 2 & 3) ────────────
    _render_revenue_recovery_simulator(df, total_revenue, rev_mult, span_days)
    _render_executive_threat_matrix(df, rev_mult, cat_perf_df)
...
def _render_revenue_recovery_simulator(df: pd.DataFrame, total_revenue: float, rev_mult: float, span_days: int):
    """
    Renders an interactive pipeline simulator for Retention, Reactivation, and Upsell.
    Computes and displays Simulated Pipeline Velocity (PVI) vs. Current.
    """
    st.markdown(
        """
        <div style='background:#0f1117;border:1px solid #1e3a5f;border-radius:12px;
             padding:14px 20px;margin-bottom:12px;'>
          <div style='font-size:13px;font-weight:800;color:#93c5fd;margin-bottom:4px;'>
            🧪 Dynamic Revenue Recovery & Pipeline Velocity Simulator (Wow Factor 2)
          </div>
          <div style='font-size:12px;color:#64748b;'>
            Model the financial impact of strategic interventions across your partner base in real-time.
          </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    with st.expander("🧪 Open Interactive Forecasting & Velocity Simulator", expanded=False):
        c_sliders, c_results = st.columns([2.5, 3.5])
        
        with c_sliders:
            st.markdown("<div style='font-size:12px;font-weight:700;color:#818cf8;margin-bottom:8px;'>Strategic Adjustments</div>", unsafe_allow_html=True)
            retention_boost = st.slider(
                "🛡️ Retention Boost (%)",
                min_value=0, max_value=100, value=10, step=5,
                help="Reduces the Churn Probability of At Risk & Critical accounts proportionally.",
                key="sim_retention"
            )
            reactivation_rate = st.slider(
                "⚡ Re-activation Rate (%)",
                min_value=0, max_value=100, value=15, step=5,
                help="Converts Critical and quiet partners back by securing a portion of their At Risk revenue.",
                key="sim_reactivation"
            )
            upsell_factor = st.slider(
                "🚀 Growth / Upsell Factor (%)",
                min_value=0, max_value=50, value=5, step=1,
                help="Boosts period sales for your high-potential Emerging and Healthy accounts.",
                key="sim_upsell"
            )
            
        with c_results:
            st.markdown("<div style='font-size:12px;font-weight:700;color:#818cf8;margin-bottom:8px;'>Impact Analytics</div>", unsafe_allow_html=True)
            
            # Recency mean (with a floor of 1 day to prevent division by zero)
            avg_recency = max(float(df["recency_days"].mean()), 1.0) if not df.empty and "recency_days" in df.columns else 30.0
            avg_churn = float(df["churn_probability"].mean()) if not df.empty and "churn_probability" in df.columns else 0.20
            
            # Pipeline Velocity Index (PVI) formula: (Total Value * (1-churn)) / Avg Recency
            pvi_orig = (total_revenue * (1.0 - avg_churn)) / avg_recency
            
            # Calculate simulated variables
            df_sim = df.copy()
            if "churn_probability" in df_sim.columns:
                df_sim["churn_prob_sim"] = df_sim["churn_probability"] * (1.0 - retention_boost / 100.0)
            else:
                df_sim["churn_prob_sim"] = 0.20
                
            # Apply upsell to emerging & healthy
            if "health_segment" in df_sim.columns:
                is_growth_tier = df_sim["health_segment"].isin({"Emerging", "Healthy"})
                df_sim["rev_sim"] = df_sim["recent_90_revenue"] * rev_mult
                df_sim.loc[is_growth_tier, "rev_sim"] = df_sim.loc[is_growth_tier, "rev_sim"] * (1.0 + upsell_factor / 100.0)
                
                # Apply reactivation to critical
                is_critical_tier = df_sim["health_segment"].isin({"Critical"}) | df_sim.get("_gone_quiet", pd.Series(False, index=df_sim.index))
                df_sim.loc[is_critical_tier, "churn_prob_sim"] = df_sim.loc[is_critical_tier, "churn_prob_sim"] * (1.0 - reactivation_rate / 100.0)
            else:
                df_sim["rev_sim"] = df_sim["recent_90_revenue"] * rev_mult
                
            # Re-evaluate simulated revenue at risk
            df_sim["rar_sim"] = df_sim["churn_prob_sim"] * df_sim["rev_sim"]
            
            # Total simulated revenue
            total_rev_sim = df_sim["rev_sim"].sum()
            
            # Churn reduction saves revenue at risk
            original_rar_sum = (df["revenue_at_risk"].sum() if "revenue_at_risk" in df.columns else 0.0) * rev_mult
            simulated_rar_sum = df_sim["rar_sim"].sum()
            saved_rar = max(0.0, original_rar_sum - simulated_rar_sum)
            
            avg_churn_sim = float(df_sim["churn_prob_sim"].mean()) if not df_sim.empty else 0.20
            pvi_sim = (total_rev_sim * (1.0 - avg_churn_sim)) / avg_recency
            
            recovered_capital = saved_rar + (total_rev_sim - total_revenue)
            
            # Render visual comparison chart
            import plotly.graph_objects as go
            fig_pvi = go.Figure()
            fig_pvi.add_trace(go.Bar(
                y=["Current Velocity", "Simulated Velocity"],
                x=[pvi_orig, pvi_sim],
                orientation='h',
                marker_color=["#475569", "#6366f1"],
                text=[f"{pvi_orig:.1f}", f"{pvi_sim:.1f}"],
                textposition='inside',
            ...
            fig_pvi.update_layout(
                ...
                font_color="#e2e8f0",
                xaxis=dict(showgrid=False, title="Pipeline Velocity Index (PVI)"),
                yaxis=dict(gridcolor='#1e2235'),
                height=130,
                margin=dict(l=10, r=10, t=10, b=10)
            )
            st.plotly_chart(fig_pvi, use_container_width=True)
            
            # Dynamic suggestion HTML cards
            st.markdown(
                f"""
                <div style='display:flex;gap:12px;margin-top:8px;'>
                  <div style='flex:1;background:#0d1e15;border:1px solid #14532d;border-radius:8px;padding:10px 14px;'>
                    <div style='font-size:10px;color:#4ade80;'>💡 PROJECTED RECOVERY</div>
                    <div style='font-size:18px;font-weight:800;color:#22c55e;'>{_fmt_inr(recovered_capital)}</div>
                  </div>
                  <div style='flex:1;background:#0f1b2b;border:1px solid #1e3a5f;border-radius:8px;padding:10px 14px;'>
                    <div style='font-size:10px;color:#93c5fd;'>📈 VELOCITY BOOST</div>
                    <div style='font-size:18px;font-weight:800;color:#3b82f6;'>+{((pvi_sim / max(pvi_orig, 1.0) - 1.0) * 100):.1f}%</div>
                  </div>
                </div>
                """,
                unsafe_allow_html=True,
...
def _render_executive_threat_matrix(df: pd.DataFrame, rev_mult: float, cat_perf_df: pd.DataFrame):
    """
    Renders a 4D Plotly Scatter Plot mapping all elite partners at risk.
    Provides an actionable playbook generator and pre-written email composer.
    """
    if df.empty:
        return
    ...
    st.markdown(
        ...
        <div style='background:#0f1117;border:1px solid #1e3a5f;border-radius:12px;
             padding:14px 20px;margin-bottom:12px;'>
          <div style='font-size:13px;font-weight:800;color:#93c5fd;margin-bottom:4px;'>
            🔮 High-Value Executive Action Threat Matrix (Wow Factor 3)
          </div>
          <div style='font-size:12px;color:#64748b;'>
            Identify and target critical interventions for accounts driving high revenue concentration.
          </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    with st.expander("🔮 Open Risk Threat Matrix & Executive Playbook Router", expanded=False):
        # 1. Identify at-risk high-revenue partners (top-20% revenue)
        rev_p80_all = float((df["recent_90_revenue"] * rev_mult).quantile(0.80)) if len(df) > 1 else 0.0
        
        threat_df = df[
            ((df["recent_90_revenue"] * rev_mult) >= rev_p80_all)
            & (df["health_segment"].isin({"At Risk", "Critical"}))
        ].copy()
        
        # Safe Fallback to ensure the chart is populated
        if threat_df.empty:
            threat_df = df[df["health_segment"].isin({"At Risk", "Critical"})].copy()
        if threat_df.empty:
            threat_df = df.head(10).copy()
            
        c_plot, c_playbook = st.columns([3, 2])
        
        with c_plot:
            import plotly.express as px
            
            # Map credit risk band to categorical colors
            color_map = {
                "High": "#ef4444",
                "Medium": "#f59e0b",
                "Low": "#22c55e",
            ...
            # Add helper columns for bubble size display
            threat_df["Scaled Revenue (₹)"] = threat_df["recent_90_revenue"] * rev_mult
            threat_df["At Risk Value (₹)"] = threat_df["revenue_at_risk"] * rev_mult
            ...
            fig_threat = px.scatter(
                ...
                y="At Risk Value (₹)",
                size="Scaled Revenue (₹)",
                color="credit_risk_band",
                color_discrete_map=color_map,
                hover_name="company_name",
                labels={
                    "churn_probability": "Churn Probability",
                    "At Risk Value (₹)": "Revenue at Risk (₹)",
                    "credit_risk_band": "Credit Band"
                },
                size_max=35,
                title="⚠️ Elite Partners - Risk Threat Matrix"
            )
            fig_threat.update_layout(
                paper_bgcolor='rgba(0,0,0,0)',
                plot_bgcolor='rgba(0,0,0,0)',
                font_color="#e2e8f0",
                xaxis=dict(gridcolor='#1e2235', range=[-0.05, 1.05], tickformat=".0%"),
                yaxis=dict(gridcolor='#1e2235'),
                height=350,
        ...
        with c_playbook:
            st.markdown("<div style='font-size:12px;font-weight:700;color:#93c5fd;margin-bottom:8px;'>Executive Playbook Router</div>", unsafe_allow_html=True)
            options = sorted(threat_df["company_name"].unique().tolist()) if not threat_df.empty else []
            if options:
                sel_p = st.selectbox("Select Partner for Playbook Integration", options, key="threat_p_select")
                
                p_row = threat_df[threat_df["company_name"] == sel_p].iloc[0]
                p_rev = p_row["recent_90_revenue"] * rev_mult
                p_rar = p_row["revenue_at_risk"] * rev_mult
                p_churn = p_row["churn_probability"]
                p_credit = p_row["credit_risk_band"]
                
                # Fetch dominant category
                partner_cats = cat_perf_df[cat_perf_df["company_name"] == sel_p] if not cat_perf_df.empty else pd.DataFrame()
                top_cat = partner_cats.loc[partner_cats["total_spend"].idxmax(), "category"] if not partner_cats.empty else "Core Categories"
                
                # Render Playbook Card
                st.markdown(
                    f"""
                    <div style='background:#12141c;border:1px solid #1e3b5f;border-left:4px solid #f59e0b;border-radius:8px;padding:12px 14px;margin-bottom:12px;'>
                      <div style='font-size:11px;font-weight:700;color:#f59e0b;'>⚠️ ACTIVE RISK PLAYBOOK: {sel_p}</div>
                      <div style='font-size:13px;font-weight:700;color:#fef2f2;margin-top:4px;'>Action: Executive Churn Mitigation & QBR Prep</div>
                      <div style='font-size:11px;color:#94a3b8;margin-top:6px;'>
                        • <b>Revenue Exposed</b>: {_fmt_inr(p_rar)} (Total: {_fmt_inr(p_rev)})<br/>
                        • <b>Key Category</b>: {top_cat}<br/>
                        • <b>Credit Standing</b>: {p_credit} Risk Band
                      </div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                ...
                # Email Composer
                st.markdown("<div style='font-size:11px;font-weight:700;color:#94a3b8;margin-bottom:4px;'>📧 Automated Executive Outreach Email Draft</div>", unsafe_allow_html=True)
                email_body = f"""Subject: Consistent AI Suite — Partnership & Inventory Review

Dear {sel_p} Leadership Team,

I hope this email finds you well. 

As part of our commitment to supporting your business, I wanted to reach out and schedule a brief review session for our partnership. We've noticed some changes in purchase patterns for {top_cat} over the last month and want to ensure we are doing everything we can to streamline logistics, optimize your pricing, and support your inventory requirements.

Are you available for a 15-minute sync this Thursday at 2:00 PM? We value our relationship and are ready to prepare specialized incentives to help accelerate your growth.

Best regards,
[Your Name]
Account Director, Consistent Suite"""
                
                st.text_area("Copy/Edit outreach draft:", value=email_body, height=140, key="threat_email_draft")
            else:
                st.info("No accounts currently flagged for risk routing.")
